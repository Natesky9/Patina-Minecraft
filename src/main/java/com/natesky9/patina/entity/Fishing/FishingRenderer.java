...
public class FishingRenderer extends EntityRenderer<FishingBobbler> {
    ...
    public FishingRenderer(EntityRendererProvider.Context pContext) {
        super(pContext);
    }

    public void render(FishingBobbler pEntity, float pEntityYaw, float pPartialTicks, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight) {
        if (!(pEntity.getOwner() instanceof Player player)) return;
        pPoseStack.pushPose();
        pPoseStack.pushPose();
    ...
    private static void vertex(
            VertexConsumer pConsumer, PoseStack.Pose pPose,
            int pPackedLight, float pX, int pY, int pU, int pV)
    {
    ...
    private static void stringVertex(float pX, float pY, float pZ, VertexConsumer pConsumer,
                                     PoseStack.Pose pPose,float pStringFraction, float pNextStringFraction) {
        ...
        f4 /= f6;
        f5 /= f6;
        pConsumer.addVertex(pPose, f, f1, f2).setColor(FastColor.ARGB32.color(255,250,255)).setNormal(pPose, f3, f4, f5);
    }
    @Override
    public ResourceLocation getTextureLocation(FishingBobbler pEntity) {
